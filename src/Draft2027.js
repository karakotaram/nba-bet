...
import { DRAFT_SERVER_CONFIGURED } from './draftServer';

const liveReady = DRAFT_SERVER_CONFIGURED || process.env.NODE_ENV === 'development';
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              ...
              {liveReady ? (
                <span className="ml-auto text-[10px] uppercase tracking-wide text-emerald-400 bg-emerald-500/10 border border-emerald-500/30 rounded px-1.5 py-0.5">Live</span>
              ) : (
                <span className="ml-auto text-[10px] uppercase tracking-wide text-amber-400 bg-amber-500/10 border border-amber-500/30 rounded px-1.5 py-0.5">Setup</span>
              )}
            </div>
